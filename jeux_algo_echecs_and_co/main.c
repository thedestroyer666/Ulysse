...
//afficher le tableau des jeux (en numérotant les cases)
    ...
    for (cptJeu=0; cptJeu<NB_JEUX; cptJeu++ )
    {
...
//remplir le tableau des participants (nom du joueur et numéro du jeu) et affecter un numéro de groupe
    ...
    int cptParticipants=0;
    for (cptPart=0; cptPart<nbreParticipants; cptPart++ )
    {
        ...
        while (numero<0 || numero>=NB_JEUX);
        strcpy(tabParticipants[cptPart].nomJ, tabJeux[numero].nom);
        if (tabJeux[numero].cptParticipants>tabJeux[numero].nbreMax) //compter le nombre de groupe
        {
            (tabJeux[numero].cptGroupes)++;
    ...
    for (cptPart=0; cptPart<nbreParticipants; cptPart++ ) //informer le participant sur son jeu
    {
        printf(" Participant[%d] %s joue a %s \n \n",cptPart, tabParticipants[cptPart].nomP, tabParticipants[cptPart].nomJ);
    }
...
//informer les participants qui font partie d un groupe dans lequel il n y a pas assez de joueurs
    ...
    for (cptPart2=0; cptPart2<=nbreParticipants-1; cptPart2++) //lancement boucle affichage
    {
        ...
        {
            if (cptPart2==0 || (tabParticipants[cptPart2].groupe!=tabParticipants[cptPart2-1].groupe))
            {
        ...
        {
            printf("     Groupe %d  \nJoueurs\n%s \n", tabParticipants[cptPart2].groupe, tabParticipants[cptPart2].nomP);
        }
        else printf("%s\n", tabParticipants[cptPart2].nomP);
    }
    numero=0;
    for (numero=0; numero<=NB_JEUX-1; numero++)
    {
        ...
        {
            printf("Le groupe %d du jeu %s ne contient pas assez de participants, revenez une prochaine fois!", tabJeux[numero].cptGroupes, tabJeux[numero].nom);
        }
    }
}
